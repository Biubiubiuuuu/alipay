...
func RefundPay(outTradeNo, tradeNo, refundReason string, refundAmount float64) (RefundResp, error) {
	...
	log.Infof("request alipay.trade.refund url:%s,req:%+v", url, postData)
	rsp, err := HTTPPost(url, postData, "")
	defer rsp.Body.Close()
	if err != nil {
		log.Errorf(err, "request alipay.trade.refund failed.")
		return resp, err
	}
	...
	log.Infof("alipay.trade.refund resp body:%+v", resp)
	return resp, nil
}
